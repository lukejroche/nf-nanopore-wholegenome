...
include { INDEX } from '/home/lroche/clinical_genomics/new_nf_ont_wholegenome/nf-core-ontwholegenome/modules/local/index/tests/../main.nf'
...
workflow {
    ...
    //run process
    INDEX.run(input.toArray())

    if (INDEX.output){

        // consumes all named output channels and stores items in a json file
        INDEX.out.getNames().each { name ->
            serializeChannel(name, INDEX.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = INDEX.out as List<Object>
        def i = 0
        array.each { output ->
